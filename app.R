...
d0 <- readxl::read_xlsx("data/risk_index_matrix.xlsx",
  sheet = 1,
  col_types = rep("text", 11)
) %>%
  mutate(Component = paste0(Component, " (", tolower(Attribute), ")")) %>%
  pivot_longer(
    cols = c(OPTION_1, OPTION_2, OPTION_3, VALUE_1, VALUE_2, VALUE_3),
...
d1 <- readxl::read_xlsx("data/risk_index_strategies.xlsx",
  sheet = 1,
...
server <-
  function(input, output, session) {
    ...
    output$y1 <- renderUI({
      myselectize(labs, v, 1)
    })
    output$y2 <- renderUI({
      myselectize(labs, v, 2)
    })
    output$y3 <- renderUI({
      myselectize(labs, v, 3)
    })
    output$y4 <- renderUI({
      myselectize(labs, v, 4)
    })
    output$y5 <- renderUI({
      myselectize(labs, v, 5)
    })
    output$y6 <- renderUI({
      myselectize(labs, v, 6)
    })
    output$y7 <- renderUI({
      myselectize(labs, v, 7)
    })
    output$y8 <- renderUI({
      if (input$y0 == "Pasture") myselectize(labs, v, 8)
    })
    ...
    output$plot <- renderPlot({
      if (length(risk()) == 0 || length(riskrating()) == 0 || is.na(riskrating())) {
        return(NULL)
      }
      risktext <- paste("Risk score =", risk(), "\n", riskrating(), "risk")
      riskcol <- c(Low = "#49b649", Moderate = "#ffed2a", High = "#ff700a")
